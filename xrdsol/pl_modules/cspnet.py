...
from xrdsol.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.coord_out = nn.Linear(hidden_dim+1, 3, bias = False)
        self.xrd_embedding = nn.Linear(4500, 1, bias = False)
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, condition_xrd):
        ...
        condition_xrd = condition_xrd.reshape(num_atoms.size(0), 4500)

        condition = self.xrd_embedding(condition_xrd)
        condition_per_atom = condition.repeat_interleave(num_atoms, dim=0)
        ...
        node_features_condition = torch.cat([node_features, condition_per_atom], dim=1)
        coord_out = self.coord_out(node_features_condition)
