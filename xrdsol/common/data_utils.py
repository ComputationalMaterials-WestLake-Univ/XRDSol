...
from pymatgen.analysis.diffraction.xrd import XRDCalculator # dongfang
from scipy.special._ufuncs import wofz # dongfang
...
def get_voigt_xrd(theta_vectors, theta_loc, intensity, alphagamma, wavelength):
    ...
    alphagamma = alphagamma * 1 / np.cos(theta_loc* 2 * np.pi / 360 / 2)
    data = Voigt(theta_vectors, theta_loc, alphagamma, intensity)  # *intensity.reshape(1,-1)
...
def process_one(row, niggli, primitive, graph_method, prop_list, use_space_group = False, tol=0.01):
    ...
    xrd = xrdcal.get_pattern(crystal, scaled=False)
